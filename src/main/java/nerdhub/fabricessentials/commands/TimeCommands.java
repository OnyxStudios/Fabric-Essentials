    ...
    public static void registerTimeCommands() {
        CommandRegistry.INSTANCE.register(false, serverCommandSourceCommandDispatcher -> ServerCommandManager.literal("day")
                ...
                .executes(context -> {
                    ...
                    context.getSource().getPlayer().addChatMessage(new TranslatableTextComponent("time.day").setStyle(new Style().setColor(TextFormat.GOLD)), true);
                    return 1;
                }));
        ...
        CommandRegistry.INSTANCE.register(false, serverCommandSourceCommandDispatcher -> ServerCommandManager.literal("night")
                ...
                .executes(context -> {
                    ...
                    context.getSource().getPlayer().addChatMessage(new TranslatableTextComponent("time.night").setStyle(new Style().setColor(TextFormat.GOLD)), true);
                    return 1;
                }));
                        ...
                        .executes(context -> {
                            ...
                            if (timePoll != null) {
                                if (!timePoll.hasPlayerVoted(playerEntity.getUuid())) {
                                    ...
                                    playerEntity.addChatMessage(new TranslatableTextComponent("time.vote_added").setStyle(new Style().setColor(TextFormat.GOLD)), true);
                                } else {
                                    context.getSource().getPlayer().addChatMessage(new TranslatableTextComponent("time.voted").setStyle(new Style().setColor(TextFormat.LIGHT_PURPLE)), true);
                                }
                            } else {
